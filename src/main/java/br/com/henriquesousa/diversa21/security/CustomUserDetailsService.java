package br.com.henriquesousa.diversa21.security;
...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    private List<String> getPrivileges(Collection<Role> roles) {
        ...
        for (Role role : roles) {
            privileges.add(role.getName()); // Add the role itself (e.g., ROLE_ADMIN)
            collection.addAll(role.getPrivileges());
        }
        for (Privilege item : collection) {
            privileges.add(item.getName()); // Add individual privileges
        }
        return privileges;
    }
